...
if not CONTINUE_OPT:
    ...
    for i in range(NUM_DIM):
        dim_range = afl.aerofoil_optimisation_domain[i]['domain'][1] - afl.aerofoil_optimisation_domain[i]['domain'][0]
        x[:,i] = x[:,i] * dim_range + afl.aerofoil_optimisation_domain[i]['domain'][0]
...
while i < EVALUATION_BUDGET:
    ...
    d1xHVI = (d1HVI - d1HVIn)
    d1xHVI_norm = normalise_f(d1xHVI, EXPLORATION_PARAM)    
